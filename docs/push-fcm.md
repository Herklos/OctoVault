# OctoVault push notifications — FCM via Whistler (native, topic-addressed)
...
- **Infra bridge:** `apps/octovault/index.ts` exports BOTH transports —
  `createOctochatSseApp()` (live SSE) and `createOctochatFcmApp(app)` (FCM push) —
  registered in `src/index.ts`, where **octobot and octovault each initialize their
  own named firebase-admin app**
  (separate Firebase projects — `FIREBASE_SERVICE_ACCOUNT` vs
  `OCTOVAULT_FIREBASE_SERVICE_ACCOUNT`); `package.json` uses
  ...
  The OctoVault **local dev** launcher (`infra/whistlers-sse.mjs`) is on `^0.6.0` too.
- **OctoVault client:** `src/lib/push/{fcm.ts,fcm.native.ts,use-push.ts}` (new),
...
`octovault_firebase_service_account` (Part B3), `eas credentials` for the iOS APNs
...
SSE has a server-side membership gate (`/v1/octovault/events` filters `?spaces=…`
...
`octovault-octovault-chat-changed-<spaceId>` via Firebase directly and keep
...
- It costs ~4-repo plumbing (a `deriveSecret` API in the keyring SDK, a meta
  ...
  `FirebaseDestination`, and a new `kickMemberFromSpace` flow in OctoVault —
...
```
drakkar-sync  ──NATS──▶  Whistler (octovault app, SSE)  ──▶  /v1/octovault/events proxy  ──▶  client
   publishes            subject: octovault.chat.changed.<spaceId>      (cap-cert auth +
...
```
drakkar-sync  ──NATS──▶  Whistler (octovault-FCM app)  ──FCM topic──▶  device
   (unchanged)          subject: octovault.chat.changed.<spaceId>     octovault-octovault-chat-changed-<spaceId>
...
`octovault` and subject `octovault.chat.changed.<spaceId>` it is exactly:
...
1. **Create a dedicated `octovault` project** at <https://console.firebase.google.com>
...
`Infra/sync/bridge/package.json` (and the OctoVault root `package.json`, used by the
...
### B1. Add the FCM factory to `src/apps/octovault/index.ts`

`apps/octovault/index.ts` holds both octovault transports — the existing
...
/**
 * OctoVault FCM relay. Same NATS subject as the SSE app, but a DISTINCT queue
 * group so NATS delivers every event to both. Per-space FCM topics come from the
 * `octovault` namespace transform: octovault-octovault-chat-changed-<spaceId>.
...
export function createOctochatFcmApp(app?: App): AppDefinition {
  return {
    ...
    subscriptions: [
      {
        name: "chat",
        topics: ["octovault.chat.changed.>"],
        group: "drakkar-bridge-fcm", // <-- distinct from the SSE app's group
      },
    ],
    createDestination: () =>
      new FirebaseDestination({
        app, // octovault's OWN named Firebase app (its dedicated project) — passed in by index.ts
        format: (n) => {
          ...
          return {
            ...
            notification: { title: "OctoVault", body: "New message in another room" },
...
octobot and octovault each push from their **own Firebase project**. A small helper
...
const octovaultApp = namedFirebaseApp("octovault", process.env["OCTOVAULT_FIREBASE_SERVICE_ACCOUNT"])
if (octovaultApp) apps.push(createOctochatFcmApp(octovaultApp))
```
...
- Add to `Infra/sync/ansible/roles/stack/templates/bridge.env.j2`:
  ```
  OCTOVAULT_FIREBASE_SERVICE_ACCOUNT={{ octovault_firebase_service_account | to_json }}
  ```
...
- Put the Part-A service-account JSON in the **ansible vault**
  (`group_vars/.../vault.yml`) as `octovault_firebase_service_account`, mirroring
...
- **No nginx / CORS** — server→FCM is a backend egress call, not a browser
  request.
- **No drakkar-sync change** — it already publishes `octovault.chat.changed.<spaceId>`.
...
should show the `octovault-fcm` app subscribing on group `drakkar-bridge-fcm`.
...
```sh
pnpm --filter @octovault/mobile add @react-native-firebase/app @react-native-firebase/messaging
pnpm --filter @octovault/mobile add expo-dev-client expo-notifications expo-build-properties
```
...
> **Merge, don't replace.** OctoVault's `app.json` already defines `plugins`,
...
const topic = (spaceId: string) => `octovault-octovault-chat-changed-${spaceId}`
...
```sh
# local dev build
pnpm --filter @octovault/mobile exec expo run:ios    # or run:android
...
3. From the Firebase console (*Messaging → New campaign → topic*) **or** an
   ...
   `octovault-octovault-chat-changed-<spaceId>` — confirm the device shows the banner.
...
an account-scoped topic `octovault-user-<userId>` (alongside its per-space topics).
When the bridge knows who authored the write, it addresses the push to an FCM
**condition** instead of a plain topic:

```
'octovault-octovault-chat-changed-<spaceId>' in topics && !('octovault-user-<authorId>' in topics)
```

Only the author's own devices are subscribed to `octovault-user-<authorId>`, so the
...
- **Server (both impls)** — OctoVault `apps/server/src/index.ts` and Infra
  ...
  `octovault.chat.changed.<spaceId>` with `identity` in the body.
- **Infra bridge** (`bridge/src/apps/octovault/format.ts`) — builds the condition when
  ...
  return {
    ...
    ...(authorId
      ? { condition: `'${n.topic}' in topics && !('octovault-user-${authorId}' in topics)` }
...
- **Client** (`fcm.native.ts`) — `pushTopicForUser(userId)` +
  ...
  `octovault-user-<session.userId>` in its reconcile effect (and drops it on
...
`bridge/src/apps/octovault/format.test.ts`).
...
- The bridge change is the `tag` + `threadId` additions in the **B1 formatter**
  above. It lives in **Infra** (`apps/octovault/index.ts`) — the diff there is
  documented but **not applied/verified from this repo**; apply + deploy on the
  Infra side (re-deploy the bridge image, Part B5).
- The OctoVault client change is **done**: `notify.ts` adds `renotify: true`
...
`sendEach`); the Infra formatter `apps/octovault/format.ts` returns `[placeholder, upgrade]`.
...
- **title shows the space + room** — `loadNotificationLabels` reads the plaintext `_rooms`
  ...
  content — so a failed/slow lookup degrades to the bare `"OctoVault"` title and never gates
...
**Bridge formatter (Infra `apps/octovault/format.ts` — DONE, committed master 3892110):**
