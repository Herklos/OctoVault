...
// window.octovault — it is here for desktop-only features. Each method wraps a
...
contextBridge.exposeInMainWorld('octovault', {
  ...
  setOverlayBadge: (png: string | null, description: string) =>
    ipcRenderer.invoke('octovault:set-overlay-badge', png, description),
  ...
  onUpdateReady: (cb: (version: string) => void) =>
    ipcRenderer.on('octovault:update-ready', (_event, version: string) => cb(version)),
  ...
  getPendingUpdate: () =>
    ipcRenderer.invoke('octovault:get-pending-update') as Promise<string | null>,
  ...
  checkForUpdates: () =>
    ipcRenderer.invoke('octovault:check-for-updates') as Promise<
      'updated' | 'current' | 'error' | 'unavailable'
    >,
  // Relaunch the app to apply a staged OTA bundle.
  relaunch: () => ipcRenderer.invoke('octovault:relaunch'),
});
